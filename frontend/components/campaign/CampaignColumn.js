...
const PageComponent = ({ campaign, openModal, closeModal }) => {
  ...
  const submitDeleteHandler = async (campaign_id) => {
    ...
    try {
      const response = await axios.delete(
        `https://resonant-petal-379617.ew.r.appspot.com/campaign/${campaign_id}/delete/`,
        config
      );
  ...
  const submitDuplicateHandler = async (campaign_id) => {
    ...
    try {
      const response = await axios.get(
        `https://resonant-petal-379617.ew.r.appspot.com/campaign/duplicate-campaign/${campaign_id}/`,
        config
      );
      window.location.reload();
      // After the project is successfully created, you can redirect to another page or give a success message.
